...
def render_search_auto(search_term):
    ...
    elif script_detector.contains_traditional_chinese(search_term) or script_detector.contains_chinese(search_term):
        entries = queries.query_traditional(search_term)
    elif script_detector.is_valid_jyutping(search_term) and queries.query_jyutping_exists(search_term):
        entries = queries.query_jyutping(search_term)
    elif script_detector.is_valid_pinyin(search_term) and queries.query_pinyin_exists(search_term):
    ...
    return render_template("dictionary_search.html", search_term=search_term,
                           search_type="search_auto", entries=entries)
